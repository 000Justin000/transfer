...
if (showpath==true)
        ...
        switch system
            ...
            case '1101'  %[126.0, -21.3]
                labels = [[   0,   0]; [ -30,  30]; [ -90,  90]; [ -35, 125]; [   0, 165]; [-25, -150]; [ -90, -105]; [-35,-65]; [ 40, -60]; [95,-105]; [150,-30]; ...
                          [  95,  80]; [ 135, 125]; [ 160, 155]; [-155, 155]; [-155, -40]; [140, -170]; [  45, 135];];
            ...
            case '10n01' %[129.8, -25.0]
                labels = [[   0,   0]; [  40, -60]; [ 90, -120]; [140, -165]; [-160, 155]; [ -90,  65]; [-85, -25]; [  0, -105]];
            case '11n01' %[106.2, -7.2]
                labels = [[   0,   0]; [  70,  -90]; [130, -145]; [-105,105]; [   0,-100]; [ -40,-145]; [-20, -165]; [ -35, 115]; [ 5,  85]];
            case '11n11' %[ 135.7, -58.7];
                labels = [[   0,   0]; [  40,  -40]; [90,-75]; [125, -120]; [165,-170]; [-140, 155]; [-80, 110]; [ -40, 60]; [ -35, -45]];
    ...
    elseif (strcmp(geoms, 'connect'))
        switch system
            case '1001'
                origin = [124.4, -22.8];
                labels = [[  0,  0]; [ 35, -75]; [100, -105]; [145, -150]; [175, 155]; [-145, 120]; [-95, 90]; [-55, 65]];                
            case '1101'
                origin = [126.0, -21.3];
                labels = [[  0,  0]; [ 40, -60]; [ 95, -105]; [140, -170]; [160, 155]; [-155, 160]; [-90, 90]; [-30,25]];
            case '1111'
                origin = [122.8, -38.4];
                labels = [[  0,  0]; [ 60, -35]; [ 95,  -90]; [125, -155]; [165, 175]; [-145, 170]; [-85, 120]; [-60, 50]];
            case '10n01'
                origin = [129.8, -25.0];
                labels = [[  0,  0]; [ 45, -65]; [ 90, -120]; [140, -165]; [-160, 155]; [-115,100]; [-95, 70]; [-60, 45]];
            case '11n01'
                origin = [106.2,  -7.2];
                labels = [[  0,  0]; [ 70, -90]; [130, -145]; [-175, 160]; [-140, 130]; [-105, 105]; [-65, 65]; [35, -45]];
            case '11n11'
                origin = [135.7, -58.7];
                labels = [[  0,  0]; [ 45, -40]; [ 90,  -75]; [125, -120]; [165, -165]; [-140, 160]; [-75, 110]; [-35, 55]];
            case '1301'
